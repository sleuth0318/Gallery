...
data class VeOverlayItem(
    ...
    /** TEXT only: 0 = left, 1 = center, 2 = right. */
    val textAlign: Int = 1,
    /** TEXT only: box background, Color.TRANSPARENT = none. */
    ...
    val scale: Float = 1f,
    /** Free-form box multipliers on top of scale, set by the border
     *  drag-handles (1 = base box). sizeX widens/narrows, sizeY stretches the
     *  box vertically. EMOJI, IMAGE and all shapes but the rectangle stay
     *  uniform (sizeY mirrors sizeX): emoji/shapes are square-locked, images
     *  keep their natural aspect; TEXT and the rectangle are free-form. */
    val sizeX: Float = 1f,
    val sizeY: Float = 1f,
    /** IMAGE only: source bitmap aspect (w/h), so the base box keeps the
     *  photo's natural shape. Other kinds leave it at 1. */
    val itemAspect: Float = 1f,
    val rotationDegrees: Float = 0f,
) {
    enum class Kind { EMOJI, SHAPE, TEXT, IMAGE }
...
data class VeBrushStroke(
    val color: Int,
    /** Stroke width as a fraction of the frame width, 0.004..0.16
     *  (M19r: max doubled, min unchanged). */
...
/** Free-form crop window in rotated space, normalized 0..1 (used by the
 *  "Free" aspect option; the opposite of the preset center-crop ratios). */
data class VeCropRect(
    val left: Float = 0f,
    val top: Float = 0f,
    val right: Float = 1f,
    val bottom: Float = 1f,
...
data class VideoEditState(
    ...
    /** Arbitrary straighten angle, -90..+90 degrees (Transform tool dotted
     *  slider). Positive = CLOCKWISE on screen. Applied after the quarter-turn
     *  rotation/flip, before the crop, auto-zoomed so no background shows. */
    val straightenAngle: Float = 0f,
    val flipHorizontal: Boolean = false,
    val flipVertical: Boolean = false,
    val aspectOption: VeAspectOption = ASPECT_FREE,
    /** Free-form crop window (normalized, rotated space); only meaningful when
     *  aspectOption == ASPECT_FREE. Cleared when a preset ratio is chosen. */
    val freeCrop: VeCropRect? = null,
    ...
    fun hasEdits(sourceDurationUs: Long): Boolean =
        trimStartUs > 0L ||
            ...
            rotationDegrees != 0f || straightenAngle != 0f || flipHorizontal || flipVertical ||
            aspectOption.ratio != null || freeCrop != null || filterIndex != 0 ||
            !adjustments.isIdentity() ||
            muted || focusMode != VeFocusMode.NONE || overlayIndex != 0 ||
            items.isNotEmpty() || strokes.isNotEmpty()

    companion object {
        // "Original" option removed per user request: the free crop leads as
        // Custom and is the default; only presets follow.
        val ASPECT_FREE = VeAspectOption("free", "Custom", null)
         ...
         *  Custom (free crop) leads; "Original" intentionally absent. */
        val ASPECT_OPTIONS: List<VeAspectOption> =
            listOf(ASPECT_FREE) + ASPECT_TOGGLES.map { (x, y) ->
