...
/** Brush color panel swatch strip: position 0 is the pipette (back to the
 *  magnifying target); the rest are fixed preset colors (color_panel.png). */
class VeHsvSwatchAdapter(
    private val colors: List<Int>,
    private val accent: () -> Int,
    private val current: () -> Int,
    private val onColor: (Int) -> Unit,
    private val onPipette: () -> Unit,
) : RecyclerView.Adapter<VeHsvSwatchAdapter.VH>() {

    class VH(parent: ViewGroup) : RecyclerView.ViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.item_ve_color_swatch, parent, false)
    ) {
        val box: View = itemView.findViewById(R.id.ve_swatch_box)
        val pipette: ImageView = itemView.findViewById(R.id.ve_swatch_pipette)
        val selected: View = itemView.findViewById(R.id.ve_swatch_selected)
    }
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        if (position == 0) {
            holder.box.setBackgroundColor(android.graphics.Color.TRANSPARENT)
            holder.pipette.visibility = View.VISIBLE
            holder.selected.visibility = View.GONE
            holder.itemView.setOnClickListener { onPipette() }
        } else {
            val color = colors[position - 1]
            holder.box.setBackgroundColor(color)
            holder.pipette.visibility = View.GONE
            val isCurrent = (color and 0x00FFFFFF) == (current() and 0x00FFFFFF)
            holder.selected.visibility = if (isCurrent) View.VISIBLE else View.GONE
            holder.selected.setBackgroundColor(accent())
            holder.itemView.setOnClickListener { onColor(color) }
        }
    }
