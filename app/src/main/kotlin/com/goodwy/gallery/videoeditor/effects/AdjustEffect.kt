...
class AdjustEffect(private val adj: VeAdjustments) : GlEffect {
    ...
    private class ShaderProgram(
        ...
        /** See CurveFilterEffect: resolve "x" vs "x[0]" driver spellings, fail loudly. */
        private fun uniformName(name: String): String {
            ...
            val resolved = when {
                ...
                else -> throw VideoFrameProcessingException("AdjustEffect: no uniform '$name'")
            }
        ...
        override fun drawFrame(inputTexId: Int, presentationTimeUs: Long) {
            try {
                ...
                glProgram.setFloatsUniform(
                    uniformName("uTexelSize"),
                    floatArrayOf(1f / width.coerceAtLeast(1), 1f / height.coerceAtLeast(1)),
                )
                val s = 1f / 100f
                glProgram.setFloatUniform(uniformName("uExposure"), adj.exposure * s)
                glProgram.setFloatUniform(uniformName("uGamma"), adj.gamma * s)
                glProgram.setFloatUniform(uniformName("uTemperature"), adj.temperature * s)
                glProgram.setFloatUniform(uniformName("uShadows"), adj.shadows * s)
                glProgram.setFloatUniform(uniformName("uHighlights"), adj.highlights * s)
                glProgram.setFloatUniform(uniformName("uWhites"), adj.whites * s)
                glProgram.setFloatUniform(uniformName("uBlacks"), adj.blacks * s)
                glProgram.setFloatUniform(uniformName("uClarity"), adj.clarity * s)
                glProgram.setFloatUniform(uniformName("uSharpness"), adj.sharpness * s)
                ...
                throw VideoFrameProcessingException(
                    "AdjustEffect: ${t.javaClass.simpleName}: ${t.message}", presentationTimeUs
                )
            }
        }
